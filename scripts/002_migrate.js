...
do $$ begin
  if not exists (select 1 from pg_policies where tablename='loyalty_members' and policyname='loyalty_members_all') then
...
do $$ begin
  if not exists (select 1 from pg_policies where tablename='transactions' and policyname='transactions_all') then
...
do $$ begin
  if not exists (select 1 from pg_policies where tablename='stamp_history' and policyname='stamp_history_all') then
...
do $$ begin
  if not exists (select 1 from pg_policies where tablename='reward_history' and policyname='reward_history_all') then
...
do $$ begin
  if not exists (select 1 from pg_policies where tablename='audit_logs' and policyname='audit_logs_all') then
...
insert into public.settings (key, value) values
  ('pricing',          '{"pricePerKg": 30, "minWeight": null, "pricingMode": "per-kg", "priceDisplayMode": "always"}'),
  ('loyalty',          '{"enabled": true, "washesPerReward": 7, "rewardDescription": "Free wash"}'),
  ('business_profile', '{"shopName": "LaundryTrack", "tagline": "We wash, you relax.", "address": "123 Magsaysay Ave, Manila", "contactNumber": "", "email": ""}'),
  ('addons',           '[{"id":"1","name":"Fabcon","rate":10,"isActive":true},{"id":"2","name":"Bleach","rate":15,"isActive":true},{"id":"3","name":"Starch","rate":20,"isActive":true}]'),
  ('load_tiers',       '[{"id":"1","name":"Small","weightRange":"below 4 kg","price":120},{"id":"2","name":"Medium","weightRange":"4–7 kg","price":180},{"id":"3","name":"Large","weightRange":"7 kg and above","price":250}]')
